...
tbequip.tbequi_tipo
FROM tbequip  
where tbequip.tbequip_id = '$tbequip_id' ORDER BY tbequip.tbequip_id";
/*/
...
//$resultado = mysqli_query($conn, $qr);
  ...
  if ($id_equip != "")
 {
    $teste = "1";
       //altera registro 
       // $query_alt = "UPDATE `tbequip` SET `tbequi_nome`='" . $nome_pc . "',`tbequip_monitor`='" . $monitor_tipo . "',`tbequi_modelomemoria`='" . $mem_mod . "',`tbequi_memoria`='" . $mem_tam . "',`tbequi_tparmazenamento`='" . $arm_tipo . "',`tbequi_armazenamento`='" . $arm_tam . "',`tbequip_plaqueta`='" . $plaqueta . "',`tbequi_arm_sec`='" . $arm_sectipo . "',`tbequi_arm_sectam`='" . $arm_sectam . "',`tbequi_slots`='" . $slots . "',`tbequi_slots_uso`='" . $slots_uso . "',`tbequip_so`='" . $so . "',`tbequi_app`='" . $app . "',`tbequi_app_out`='" . $app_outros . "',`tbequi_obs`='" . $obs . "',`tbequi_teclado`='" . $teclado . "',`tbequi_mouse`='" . $mouse . "',`tbequi_filtrodelinha`='" . $filtrodelinha . "',`tbequip_monitor_num`='" . $monitor_num . "',`tbequip_fonte`='" . $fonte . "',`tbequip_lacre`='" . $lacre . "',`tbequip_resp`='" . $resp . "',`tbequip_util_nomes`='" . $util_nomes . "',`tbequip_util_num`='" . $util_num . " ' WHERE `tbequip_id`='" . $id_equip . "' ";
    ...
    if ($result == 0) {
        ...
        echo "Dados do equipamento  ".$id_equip . " controle T.I. " . $ctrl_ti . "<br>";
        echo "Base  do equipamento  ".$idb . " controle T.I. " . $ctrl_tib . "<br><br>";
       
        if (strlen(preg_replace('/\s\s+/', ' ', $idb)) <> (strlen(preg_replace('/\s\s+/', ' ', $id_equip)))) {  // registra a informacao de alteraçao e guarda a da base  function registra_alt($Fconexao, $Fctrl_ti,$Fid, $Ftabela, $Fcampo,$Fd_b, $Foutros,$Fusuario  )/
            $campo = "Ref_id";
            $dados = $idb . " --> " . $id;
            echo $campo."  ".$dados . "<br>";
