...
if($nivel_usuario != '1'){
  echo "<script>alert('Usuario não pode salvar arquivos');</script>";
...
if($_FILES) { // Verificando se existe o envio de arquivos.
    ...
    if ($name !=""){
      $result = mysql_query('SELECT * FROM arquivo where arquivo = "'.$name.'" ');
    if ($result){
  ...
  mysql_free_result($result);
    ...
    $sqlinsert  = "INSERT INTO  arquivo(arquivo, id_tipo_arquivo) VALUES('$name', '$tipo_documento');";
